package com.hndl.cn.mghd.sys.business.resources.service.impl;
...
public class ResourcesServiceImpl implements ResourcesService {
    ...
    public Result<Object> uploadTemporaryBase64Img(String base64) {
        try{
            ...
            if (StringUtils.isBlank(base64)) {
                throw new MghdSysException(MghdSysExceptionEnums.MGHD_SYS_PIC_IS_NULL_EXCEPTION.getCode());
            }
            if (!uploadFilePattern.matcher(base64.substring(0,20)).matches()) {
                throw new MghdSysException(MghdSysExceptionEnums.MGHD_SYS_FILE_ILLEGAL_EXCEPTION.getCode());
            }
            ...
            throw new MghdSysException(MghdSysExceptionEnums.MGHD_SYS_PIC_UPLOAD_FAIL_EXCEPTION.getCode());
        }
    }
    ...
    public Result<Object> uploadTemporaryImg(InputStream io, String fileExtensionName) {
        try{
            //校验图片格式是否符合上传格式
            if (!uploadFilePattern.matcher(fileExtensionName).matches()) {
                throw new MghdSysException(MghdSysExceptionEnums.MGHD_SYS_FILE_ILLEGAL_EXCEPTION.getCode());
            }
     ...
     */
    @Override
    public String registerTopicCommentImg(String topicId, String commentImgUrl) {
        String path = klfshAliyunOssConfig.getBasePath() + "/" + klfshAliyunOssConfig.getTmpDir();
        if(!commentImgUrl.contains(path)){
            throw new MghdSysException(PublicExceptionCodeEnum.Ex_PARAM_ERROR.getCode());
        }
        return aliyunOssUploadService.copyTopicCommentImg(topicId,commentImgUrl);
    }

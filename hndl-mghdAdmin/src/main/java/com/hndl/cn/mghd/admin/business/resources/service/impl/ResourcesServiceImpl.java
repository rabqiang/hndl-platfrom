package com.hndl.cn.mghd.admin.business.resources.service.impl;
...
public class ResourcesServiceImpl implements ResourcesService {
    ...
    public Result<Object> uploadTemporaryBase64Img(String base64) {
        try{
            ...
            throw new MghdAdminException(MghdAdminExceptionEnums.MGHD_SYS_PIC_UPLOAD_FAIL_EXCEPTION.getCode());
        }
    }
     ...
     */
    @Override
    public String registerTopicImg(String topicImgUrl) {

        //校验图片前缀
        String path = klfshAliyunOssConfig.getBasePath() + "/" + klfshAliyunOssConfig.getTmpDir();
        if(!topicImgUrl.contains(path)){
            throw new MghdAdminException(PublicExceptionCodeEnum.Ex_PARAM_ERROR.getCode());
        }
        return aliyunOssUploadService.copyTopicImgUrl(topicImgUrl);
    }
     ...
     */
    @Override
    public Result<Object> uploadTopicImg(InputStream inputStream, String fileExtensionName) {
        try{

            String fileName ="topic/content/"+LocalDate.now().toString()+"/" +  snowflakeIdService.nextFileId()  + "." + fileExtensionName;

            return ResultFactory.success(aliyunOssUploadService.putTopicObject(fileName,inputStream));
        }catch (Exception e){
            throw new MghdAdminException(MghdAdminExceptionEnums.MGHD_SYS_PIC_UPLOAD_FAIL_EXCEPTION.getCode());
        }
    }
